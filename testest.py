...
class controllsystem: # contains all the pid controlles in the first block of the dynamic system aswell as the truncating limiters
    ...
    def __init__(self,p1,p2,p3,t1,t2,t3,tc,dt): # sets up pid controllers with values from p1,p2,p3
        ...
        self.altpid = PID(4.5,0,7.5,0.000,0,20/1000,3500,-2500,4500)
        self.rollpid = PID(0.8,0,1.2,0.000,0,20/1000,100,(-100),580)
        self.pitchpid = PID(0.8,0,1.2,0.000,0,20/1000,100,(-100),580)
    ...
    def update(self , meas,sett): # take the xyz cordinate
        ...
        self.altpid.Step(meas[2], sett[2])
...
class AltHoldExample: # controlls the connection to the dron and vicon
    ...
    pointstates = [[0, 0, 1000], [1000, 0, 1000], [1000, 0, 1700], [0, 0, 1700], [0, 0, 1000],] # path test
    def _stab_log_data(self, timestamp, data, logconf):
        ...
        if self.land==False:
            ...
            if(thval > int16max):
                thval=int16max
            self._cf.commander.send_setpoint(-np.rad2deg(retvar[1] / 588), np.rad2deg(retvar[0] / 588), 0,thval)  # send the generated comand to the drone

            print("retvar "+str(-np.rad2deg(retvar[1] / 588))+" , "+str(np.rad2deg(retvar[0] / 588))+" thrust: "+ str(int(retvar[2])*8) + ": pos was:" + str(VIPOS))
            ...
            #  maby think of of inverse clamping the sensitivety of the angular controls by how far the curent thrust is above hover
            # self._cf.commander.send_setpoint(np.rad2deg(-retvar[0]/588) ,-np.rad2deg(retvar[1]/588) ,0,int(retvar[2])*13) # send the generated comand to the drone
            self._cf.commander.send_setpoint(-np.rad2deg(retvar[1] / 588), np.rad2deg(retvar[0] / 588), 0, 42000 + int(retvar[2]) * 6)  # send the generated comand to the drone
    ...
    def _stab_log_data_targetland(self, timestamp, data, logconf):
        ...
        if self.land == False:
            ...
            if(self.pointstate==0): # takeoff
                ...
                target = [TARG[0],TARG[1],TARG[2]+100]
            elif(self.pointstate==3): #land
                target = [TARG[0],TARG[1],TARG[2]+80]
                if np.sqrt(
            ...
            #  maby think of of inverse clamping the sensitivety of the angular controls by how far the curent thrust is above hover
            # self._cf.commander.send_setpoint(np.rad2deg(-retvar[0]/588) ,-np.rad2deg(retvar[1]/588) ,0,int(retvar[2])*13) # send the generated comand to the drone
            self._cf.commander.send_setpoint(-np.rad2deg(retvar[1] / 588), np.rad2deg(retvar[0] / 588), 0,
                                             45500 + int(retvar[2]) * 6)  # send the generated comand to the drone

            print("retvar " + str(-np.rad2deg(retvar[1] / 588)) + " , " + str(
                np.rad2deg(retvar[0] / 588)) + " thrust: " + str(int(retvar[2]) * 8) + ": pos was:" + str(VIPOS))
